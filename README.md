...
- Project Settings for shared non-secret Discord Application ID/public key, Vercel project, Supabase project ref, and production URL; release tooling refuses cross-project drift
...
Inside an exported folder, `npm run release:activity -- --env-file .env.activity.local` reads the public project targets from Unreal and prints the complete release plan without changing either service. Older exports without targets can add `--supabase-project-ref YOUR_REF --vercel-project YOUR_PROJECT`. Add `--apply` only after reviewing the selected projects. Windows, macOS, and Linux use the same command.

For the lowest-friction path, first choose **Tools → HTML5 Export → Discord Activity Project Settings…** and enter this game's public target identity. These values are saved in the Unreal project's `DefaultGame.ini`, so the same Discord/Vercel/Supabase targets follow the project across Windows, Linux, and macOS. Then choose **Export Discord Activity…**. It runs the readiness gate before exporting, reports the exact supported/unsupported Blueprint-node counts, and offers to open the finished folder. Every export includes `activity-handoff.json`; it says `unreal-export-complete` only when every exported Blueprint node is covered, otherwise it says `unreal-export-needs-blueprint-adapters` and points to the exact nodes in `logic/blueprints.json`. The release command reads the public targets from that handoff and rejects conflicting CLI arguments or environment identities.

Only public target identity belongs in Unreal Project Settings: Discord Application ID, Discord Public Key, Vercel Project Name, Supabase Project Ref, and an optional public production URL. Discord client secrets/bot tokens, Supabase secret or signing keys, and the Activity state secret remain in the release operator's gitignored environment and Vercel. Deployment and two-client checks also remain owned by the release operator.
