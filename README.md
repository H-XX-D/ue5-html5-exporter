...
- A Blueprint-only `Web_<Function>` fallback convention for impure synchronous user C++ calls, including calls with connected output pins, so Unreal teams can rebuild portable behavior in familiar Blueprint graphs before reaching for JavaScript
...
| User C++ gameplay | Impure synchronous calls can redirect to an exported `Web_<Function>` Blueprint function and return connected output values before execution continues; pure or native-only behavior uses project-owned JavaScript adapters |
...
For an impure synchronous project C++ call, run **Tools → HTML5 Export → Create Blueprint Web Fallback Drafts…**. Unreal audits the same export scope, creates `Web_<Function>` in the owning Blueprint, and copies the original input and output pin names and types. Rebuild the portable behavior with supported Blueprint nodes, set every required output on the generated Function Result, test it, then delete the large orange `UE5HTML5 DRAFT FALLBACK` comment. The exporter deliberately leaves the original call unsupported while that marker exists; after deletion it passes the original inputs into the function, captures its returned values synchronously, and reports the call as Blueprint-fallback-covered. The operation supports Undo and never saves Blueprint assets automatically. Pure functions—and behavior that genuinely needs browser APIs or native libraries—still use **Open Custom Web Adapters Folder**. Declare the Unreal function name in `custom-adapters.json`, implement it in `custom-adapters.js`, and keep both files in source control. The fast audit and handoff distinguish built-in, completed Blueprint-fallback, Blueprint-draft-candidate, project-adapter, and uncovered nodes. The browser refuses to start Blueprint logic if any declared JavaScript implementation failed to register. Registration is a wiring check, not behavior certification, so project-adapter coverage still requires local Discord preview and gameplay testing.
...
These adapters intentionally reproduce portable gameplay behavior, not Unreal's engine internals. Chaos rigid-body determinism, authoritative Unreal replication/ownership/validation, full GAS prediction, Behavior Tree decorators/services, exact Slate layout, Niagara scripts, Sound Cues/procedural audio graphs, and exact compiled-C++ behavior still need project-specific portable replacements. Eligible impure synchronous C++ calls can keep that replacement in Blueprint through `Web_<Function>`, including synchronous outputs; pure or native-only behavior still needs a web adapter. See [Runtime adapters](docs/RUNTIME_ADAPTERS.md) for the API and exact boundary.
