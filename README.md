...
Inside an exported folder, `npm run release:activity -- --env-file .env.activity.local --vercel-only-secrets` reads the public project targets from Unreal and prints the complete release plan without changing either service. The included Windows, macOS, and Linux launchers add `--vercel-only-secrets` automatically. On `--apply`, missing server secrets are entered through hidden terminal prompts, kept in process memory, and sent directly to Vercel through stdin; they are never written into the local env file. Older exports without targets can add `--supabase-project-ref YOUR_REF --vercel-project YOUR_PROJECT`.
...
Only public target identity belongs in Unreal Project Settings: Discord Application ID, Discord Public Key, Vercel Project Name, Supabase Project Ref, and an optional public production URL. Discord client secrets/bot tokens and Supabase secret/signing keys can remain Vercel-only; the guided launcher prompts for them without echoing or saving them locally, and generates the Activity state secret in memory. Deployment and two-client checks also remain owned by the release operator.
