...
function renderAssetStorage(detail = assetPackCache?.persistence) {
  ...
  const messages = {
    ...
    persistent: 'Protected. Unchanged verified assets can be reused across game updates, and the browser will not automatically evict this Activity origin’s cache or game-created local saves.',
    'best-effort': 'Unchanged verified assets can be reused across game updates, but the browser may remove the cache when storage is tight.',
