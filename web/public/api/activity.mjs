...
const LIVE_CERTIFICATION_SCHEMA = 'ue5-discord-live-certification/v2';
const LIVE_CERTIFICATION_REQUIRED_CLIENTS = 2;
const LIVE_CERTIFICATION_COHORT_SECONDS = 10;
...
function liveCertificationCohortKey(env, instanceKey, now = Date.now()) {
  const cohort = Math.floor(now / (LIVE_CERTIFICATION_COHORT_SECONDS * 1000));
  return opaqueStateId(env, 'live-certification-cohort', `${instanceKey}:${cohort}`);
}

async function certifyLiveSession(request, body, admin, fetchImpl, env, proxyAuthenticated, now) {
  const verified = await verifyActivitySession(request, body, fetchImpl, env);
  if (verified.error) return verified.error;
  const participantCount = new Set(
    Array.from(verified.instance.users || [], (value) => String(value)),
  ).size;
  const challengeKey = liveCertificationCohortKey(env, verified.instanceKey, now);
  const { data, error } = await admin.rpc('check_in_discord_activity_certification_v2', {
  ...
  return json({
    ...
    serverBoundCohort: true,
    simultaneousPollingRequired: true,
    proxyAuthenticationEnforced: Boolean(data.all_proxy_authenticated),
    realtimeRequired: false,
    certificationWindowSeconds: LIVE_CERTIFICATION_COHORT_SECONDS,
    privacy: {
      ...
      retention: 'opaque check-ins older than 24 hours are deleted during certification',
    },
  });
}
...
export async function handleActivityRequest(request, {
  ...
  try {
    ...
    if (body.action === 'certify-live') {
      return certifyLiveSession(request, body, admin, fetchImpl, env, proxyVerification.enforced, now);
    }
