...
Native C++ cannot be translated safely from compiled Unreal modules. For a synchronous C++ call, first use the Blueprint-only fallback convention. This includes impure calls with or without outputs and pure calls whose result is connected to a consumer:

1. Keep the existing C++ call, for example `NativeApplyDamage`, in the graph.
2. Choose **Tools → HTML5 Export → Create Blueprint Web Fallback Drafts…**. The plugin creates `Web_NativeApplyDamage` in the same Blueprint and copies the native call's visible input and output pin names and types, including visible split pins.
3. Rebuild the portable behavior with ordinary supported Blueprint nodes and set every required output on the generated Function Result. A pure original produces a pure fallback; keep that graph deterministic and side-effect-free. A manually created `Web_` function for a pure original must also have **Pure** enabled. The large orange `UE5HTML5 DRAFT FALLBACK` comment deliberately keeps the original call uncovered during this work.
...
Draft creation is explicit, transactional, undoable, and does not save Blueprint assets automatically. Repeated runs do not duplicate an existing draft. The compatibility IR labels eligible uncovered calls with `repairKind: "blueprint-fallback-draft"` and `suggestedBlueprintFunction`; a named fallback that violates a pure original's contract is instead labeled `repairKind: "blueprint-fallback-purity"` with the exact correction. Package preflight independently checks the candidate count carried by the manifest and handoff. The exporter writes `webFallbackFunction: "Web_NativeApplyDamage"` only after the marker is removed and adds `webFallbackReturnsValue` when the original call consumes an output. For an impure call, the browser passes the inputs into the fallback, captures and caches its Function Result values, and only then follows the caller's execution wire. For a pure call, it evaluates the pure fallback on demand and returns the matching output to the consuming data pin. A missing synchronous Function Result is a runtime error rather than a silent default. No generated web file or JavaScript is edited. Exact C++ semantics, native libraries, operating-system APIs, and behavior that cannot complete synchronously still need a JavaScript project adapter.
