...
Before creating or configuring any hosted service, choose **Tools → HTML5 Export → Quick Start Discord FPS Preview** for a First Person-style level. The plugin selects an existing browser target or asks before adding an undoable practice target, then exports the open level to `Saved/UE5HTML5/DiscordActivityPreview`, uses Unreal's bundled Python to serve it on loopback, and opens the browser with an explicit local-only preview flag. Choose **Export & Preview Discord Blueprint Logic** when the level does not need target setup. The runtime then uses Discord's official `DiscordSDKMock`, a synthetic participant, looped-back Broadcast, mock entitlements, and revisioned browser-local game state. It never calls Discord, Vercel, Supabase, OAuth, or billing.
...
The generated **Logic** panel includes **Keep this game downloaded → Protect cached assets**. This player-initiated control requests persistent browser storage for the current Activity origin; it does not install native files, inspect device quota, or store identity. A browser can deny or omit the API, in which case the verified Cache API and network fallback continue normally. A grant protects the origin's reusable assets and game-created local saves from automatic browser eviction, while users retain control through browser or Discord storage settings.

Direct mono/stereo Unreal `SoundWave` assets used by **Play Sound 2D** or **Play Sound at Location** are exported into that same verified asset pack and decoded in the browser. Audio creates no player profile or device record. The local preview can verify that a gesture unlocks playback, but release testing must also cover a real Discord voice call and representative mobile clients because Activity audio shares the client's resources and browser autoplay restrictions. Positional playback uses camera-relative Web Audio HRTF panning with a 2D fallback when panning is unavailable; Sound Cues, MetaSounds, attenuation assets, and procedural audio remain project-adapter work.
...
When that audit finds an eligible synchronous uncovered call, choose **Create Blueprint Web Fallback Drafts…**. Eligibility includes impure calls and pure calls with a connected result. The plugin creates the correctly named `Web_<Function>` graph in the owning Blueprint and copies its visible input and output pins. A visible orange draft comment keeps the call uncovered in the compatibility report, manifest, handoff, and package preflight until the developer rebuilds the portable behavior, sets every required Function Result output, tests it, and deletes the marker. Pure originals produce pure fallbacks and must remain deterministic and side-effect-free; a manually created fallback must have **Pure** enabled to cover a pure original. Impure return values are captured before the original caller continues. The operation is transactional, supports Undo, and never saves Blueprint assets automatically. Native libraries, operating-system APIs, and nonsynchronous behavior remain project-adapter work.
...
For production, `--environment production --apply` still creates a staged deployment with `--skip-domain` and never promotes it. The separate production launcher supplies the additional explicit `--promote` option. Its dry-run plan and yes/no prompt both name the live change; after confirmation, the workflow verifies the exact staged URL plus separate SHA-256 identities for the complete export manifest and reusable asset pack before promotion. It then verifies that the configured stable Production URL serves both identities. The manifest identity distinguishes releases whose game assets happen to be unchanged. A failed staged check or missing identity cannot invoke promotion. Use `--no-migrate` or `--no-deploy` when an operator intentionally owns that step separately; `--promote` is rejected with `--no-deploy` or a Preview environment.
...
- `assetPack` manifest/handoff fields: a SHA-256 resource index and delivery policy used by the viewer's origin-scoped, content-addressed Cache API and versioned module loader. Every managed non-HTML network request carries the full pack hash, while unchanged Cache API resources are reused by their individual digest across exports. Browser eviction or unavailable storage falls back to network loading; no native client install or service worker is required.
...
Use a Preview deployment for Discord testing. For a controlled release, double-click `release-discord-activity-production.cmd` on Windows, `release-discord-activity-production.command` on macOS, or run `./release-discord-activity-production.sh` on Linux. The equivalent manual sequence is to stage production without assigning the domain, test that exact deployment, then promote it:
...
8. Launch the Activity in a private test server. Open **Logic → Run two-client check** on two clients signed into different Discord accounts in the same Activity instance. Keep both checks running together; either client will offer `discord-live-certification.json` after the backend has rechecked both accounts against Discord's Activity Instance API during the same server-bound ten-second polling cohort. The certificate stores and reports no raw player identity: Supabase receives only HMAC instance/player/cohort keys, participant counts, booleans, and timestamps; stale rows older than 24 hours are removed when a certification check runs. A recent check-in from another run or repeatedly checking from one account cannot satisfy the gate.
9. Keep the certificate with the export evidence, then manually verify the behavior outside that narrow gate:
...
The live certificate proves two distinct authenticated accounts actively polled in one Discord Activity instance during the same server-bound ten-second cohort and that Discord reported at least two instance participants. It does not prove frame-perfect simultaneity, Broadcast/game-state convergence, reconnect recovery, account-isolated saves, mobile layout/performance, purchases, or native Unreal compilation. Record those results separately instead of treating the certificate as an all-purpose release approval.
...
const liveCertificate = await activity.certifyLiveSession({
  onProgress: ({ authenticatedClientCount }) => console.log(`${authenticatedClientCount}/2 checked in`),
});
```
