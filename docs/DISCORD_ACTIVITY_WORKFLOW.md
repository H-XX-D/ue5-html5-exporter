...
When that audit finds an impure uncovered call with no connected data output, choose **Create Blueprint Web Fallback Drafts…**. The plugin creates the correctly named `Web_<Function>` graph in the owning Blueprint and copies its input pins. A visible orange draft comment keeps the call uncovered in the compatibility report, manifest, handoff, and package preflight until the developer rebuilds the portable behavior, tests it, and deletes the marker. The operation is transactional, supports Undo, and never saves Blueprint assets automatically. Pure calls, connected return values, native libraries, and operating-system APIs remain project-adapter work.
