...
8. Launch the Activity in a private test server. Open **Logic → Run two-client check** on two clients signed into different Discord accounts in the same Activity instance. Either client will offer `discord-live-certification.json` after the backend has rechecked both accounts against Discord's Activity Instance API. The certificate stores and reports no raw player identity: Supabase receives only HMAC instance/player/challenge keys, participant counts, booleans, and timestamps; stale rows older than 24 hours are removed when a certification check runs. Repeating the action from one account cannot satisfy the two-account gate.
