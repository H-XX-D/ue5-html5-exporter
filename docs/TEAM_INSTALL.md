...
For an action-style user C++ call such as `NativeApplyDamage`, add an impure Blueprint function named `Web_NativeApplyDamage` in the same Blueprint and reproduce the browser-safe behavior with supported Blueprint nodes. The exporter routes the original call there and reports it separately as Blueprint-fallback-covered. This no-JavaScript path is intentionally limited to calls with no connected data outputs; pure and return-valued C++ still use project web adapters.
