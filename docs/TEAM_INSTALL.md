...
For a synchronous user C++ call such as `NativeApplyDamage` or a pure score calculation with a connected result, choose **Create Blueprint Web Fallback Drafts…**. The plugin creates the matching `Web_` function in the owning Blueprint with the same visible input and output pins. Reproduce the browser-safe behavior with supported nodes, set every required Function Result output, test it, and delete the orange draft marker. Keep generated pure fallbacks deterministic and side-effect-free. Until marker deletion the exporter keeps the original call uncovered, preventing an empty function from creating false compatibility. Native libraries and nonsynchronous behavior still use project web adapters.
