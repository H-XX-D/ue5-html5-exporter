...
For an action-style user C++ call such as `NativeApplyDamage`, choose **Create Blueprint Web Fallback Drafts…**. The plugin creates `Web_NativeApplyDamage` in the owning Blueprint with matching input pins. Reproduce the browser-safe behavior with supported nodes, test it, and delete the orange draft marker. Until that deletion the exporter keeps the original call uncovered, preventing an empty generated function from creating false compatibility. This no-JavaScript path is intentionally limited to calls with no connected data outputs; pure and return-valued C++ still use project web adapters.
