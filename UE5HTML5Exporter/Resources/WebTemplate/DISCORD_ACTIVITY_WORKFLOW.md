...
When that audit finds an impure synchronous uncovered call, choose **Create Blueprint Web Fallback Drafts…**. The plugin creates the correctly named `Web_<Function>` graph in the owning Blueprint and copies its input and output pins. A visible orange draft comment keeps the call uncovered in the compatibility report, manifest, handoff, and package preflight until the developer rebuilds the portable behavior, sets every required Function Result output, tests it, and deletes the marker. Connected return values are captured before the original caller continues. The operation is transactional, supports Undo, and never saves Blueprint assets automatically. Pure calls, native libraries, operating-system APIs, and nonsynchronous behavior remain project-adapter work.
