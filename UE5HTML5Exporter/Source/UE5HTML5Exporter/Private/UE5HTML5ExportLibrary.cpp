...
namespace
{
    ...
    bool WriteManifest(const FString& OutputDirectory, UWorld* World, const TSet<AActor*>& SelectedActors, FUE5HTML5ExportResult& Result)
    {
        ...
        Unsupported.Add(MakeShared<FJsonValueString>(TEXT("Compiled user C++ cannot transfer directly; eligible synchronous calls may use Web_<Function> Blueprint fallbacks, including pure calls with connected results and impure calls with or without results")));
    ...
    const FString& OutputDirectory)
{
    ...
    if (Report.BlueprintRepairCandidateNodeCount > 0)
    {
        ...
        Text += TEXT("Run Tools > HTML5 Export > Create Blueprint Web Fallback Drafts to create these functions with matching visible input and output pins. Each remains unsupported while its orange draft marker exists. Rebuild the behavior, keep pure fallbacks deterministic and side-effect-free, set every required output, delete the marker, and rerun this audit.\n\n");
    }
