...
/**
 * Add this interface to a Blueprint to receive Discord lifecycle, display,
 * multiplayer, participant, and entitlement events after HTML5 export. Native
 * Unreal play remains inert.
 */
...
public:
    UFUNCTION(BlueprintImplementableEvent, Category = "UE5 HTML5|Discord Activity|Lifecycle")
    void DiscordActivityConnectionStateChanged(const FString& StateName);

    UFUNCTION(BlueprintImplementableEvent, Category = "UE5 HTML5|Discord Activity|Lifecycle")
    void DiscordActivityReady();

    UFUNCTION(BlueprintImplementableEvent, Category = "UE5 HTML5|Discord Activity|Lifecycle")
    void DiscordActivityUnavailable(const FString& Reason);

    UFUNCTION(BlueprintImplementableEvent, Category = "UE5 HTML5|Discord Activity|Lifecycle")
    void DiscordActivityError(const FString& ErrorCode, const FString& ErrorMessage);

    UFUNCTION(BlueprintImplementableEvent, Category = "UE5 HTML5|Discord Activity|Lifecycle")
    void DiscordActivityWarning(const FString& WarningCode, const FString& WarningMessage);
